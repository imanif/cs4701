...
def run_toy_task(
    data_iter: Iterator[Any], lattice: last.RecognitionLattice, num_steps: int = 200
) -> dict[str, Any]:
    ...
    def eval_step(params, batch):
        ...
        alignment_labels, _, path_weights = lattice.apply(
            params, frames=frames, num_frames=num_frames, method=lattice.shortest_path
        )
        sequence_loss = lattice.apply(params, *batch[1:])

        def masked(labels, num_labels):
            mask = torch.arange(labels.shape[-1]) < num_labels[..., None]
            return torch.where(mask, labels, 0)
    ...
    for i in range(num_steps):
        ...
        logging.info("step %d loss %s", i, torch.get_device(loss))
...
def gen_exact_copy(seed=0, batch_size=16, min_length=8, max_length=12):
    ...
    while True:
        labels = 1 + rng.randint(VOCAB_SIZE, size=(batch_size, max_length)).astype(
            np.int32
        )
        num_labels = rng.randint(min_length, max_length + 1, size=(batch_size,)).astype(
            np.int32
        )
...
def gen_odd(seed=0, batch_size=16, min_length=8, max_length=12):
    ...
    torch.testing.assert_close(translate([1, 2, 3, 4, 5]), torch.testing([1, 3, 5]))

    rng = np.random.RandomState(seed)
    embed = rng.normal(size=(VOCAB_SIZE, 64)).astype(torch.float32)
    while True:
        ilabels = 1 + rng.randint(VOCAB_SIZE, size=(batch_size, max_length)).astype(
            torch.int32
        )
        frames = embed[ilabels - 1]
        num_frames = rng.randint(min_length, max_length + 1, size=(batch_size,)).astype(
            torch.int32
        )

        labels = torch.zeros_like(ilabels)
        num_labels = torch.zeros((batch_size,), torch.int32)
...
def gen_odd_bigram(seed=0, batch_size=16, min_length=8, max_length=12):
    ...
    torch.testing.assert_close(translate([1, 2, 3, 4, 5]), [2, 6, 4])
...
def gen_next_label(seed=0, batch_size=16, min_length=8, max_length=12):
    ...
    torch.testing.assert_close(translate([1, 2, 3, 4, 5]), [2, 3, 4, 5, 5])

    rng = np.random.RandomState(seed)
    embed = rng.normal(size=(VOCAB_SIZE, 64)).astype(np.float32)
    while True:
        ilabels = 1 + rng.randint(1, VOCAB_SIZE, size=(batch_size, max_length)).astype(
            np.int32
        )
        num_frames = rng.randint(min_length, max_length + 1, size=(batch_size,)).astype(
            np.int32
...
def gen_next_label_bigram(
    seed=0, batch_size=16, min_length=8, max_length=12, stack_input=True
):
    def translate(ilabels):
        olabels = []
    ...
    torch.testing.assert_close(translate([1, 2, 3, 4, 5]), [4, 6, 8, 2, 5])
...
def gen_next_even(seed=0, batch_size=16, min_length=8, max_length=12):
    ...
    torch.testing.assert_close(translate([1, 1, 2, 3, 5, 8, 0]), [1, 5, 8])
    torch.testing.assert_close(translate([0, 1, 2, 3, 4, 5, 6]), [1, 3, 5])
    torch.testing.assert_close(translate([0, 2, 4, 6]), [0, 2, 4])
...
def gen_copy_no_repeat(seed=0, batch_size=16, min_length=8, max_length=12):
    ...
    torch.testing.assert_close(translate([1, 2, 3, 3, 4]), [1, 2, 3, 4])
...
def gen_odd_twice_no_repeat(seed=0, batch_size=16, min_length=8, max_length=12):
    ...
    torch.testing.assert_close(translate([1, 2, 3, 3, 5]), [1, 1, 3, 3, 5, 5])
...
def model_factory(
    ...
    alignment: last.alignments.TimeSyncAlignmentLattice = last.alignments.FrameDependent(),
...
def assert_same_labels(a, b):
    for i in range(max(len(a), len(b))):
        npt.assert_array_equal(
            a[i][np.nonzero(a[i])], b[i][np.nonzero(b[i])], err_msg=f"row index = {i}"
        )
