...
class RecognitionLattice(nn.Module, Generic[T]):
    ...
    -   Weight function: The neural network that produces arc weights from any
        context state given an input frame. See pylast.weight_fns for details.
    ...
    -   Start state: (0, s_a, s_c), where s_a is the start state in the
        ...
        pylast.contexts.TimeSyncAlignmentLattice), and s_c is the start state in the
        context dependency.
    ...
    -   A locally normalized model uses pylast.weight_fns.LocallyNormalizedWeightFn,
    ...
    def __init__(
        ...
        weight_fn_cacher_factory: Callable[
            [contexts.ContextDependency], weight_fns.WeightFnCacher[T]
        ],
        weight_fn_factory: Callable[
            [contexts.ContextDependency], weight_fns.WeightFn[T]
        ],
    ) -> None:
        super().__init__()
        ...
        self.weight_fn_cacher = weight_fn_cacher_factory(context)
        self.weight_fn = weight_fn_factory(context)
    ...
    def build_cache(self) -> T:
        ...
        avoid unnecessary recomputation (see pylast.weight_fns for more details).
    ...
    def forward(
        ...
        if frames.shape[:-2] != batch_dims:
            raise ValueError(
                "frames and num_frames have different batch_dims: "
                f"{frames.shape[:-2]} vs {batch_dims}"
            )
        if labels.shape[:-1] != batch_dims:
            raise ValueError(
                "labels and num_frames have different batch_dims: "
                f"{labels.shape[:-1]} vs {batch_dims}"
            )
        if num_labels.shape != batch_dims:
            raise ValueError(
                "num_labels and num_frames have different batch_dims: "
    ...
    def shortest_path(
        self, frames: torch.Tensor, num_frames: torch.Tensor, cache: Optional[T] = None
        ...
        "shorest" path under the max-tropical semiring.
        ...
        # semiring.
        def forward(lattice: "RecognitionLattice", lexical_mask: torch.Tensor):
            path_weights, _ = lattice._forward(  # pylint: disable=protected-access
                ...
                lexical_mask=[
                    lexical_mask[..., i, None, :] for i in range(num_alignment_states)
                ],
            )
            return path_weights
        ...
        path_weights = torch.autograd.grad(lexical_mask)
        viterbi_lexical_mask = torch.autograd.backward(torch.ones_like(path_weights))
        is_blank = torch.all(viterbi_lexical_mask == 0, dim=-1)
        alignment_labels = torch.where(
            is_blank, 0, 1 + torch.argmax(viterbi_lexical_mask, dim=-1)
        )
    ...
    def _string_forward(
        ...
        # arcs, thus we use a scan loop over the (max_num_labels + 1) dim to
        ...
        # max_num_frames dim because weight_fn can be vectorized over multiple
        ...
        def compute_weights(weight_fn, frame, state):
            state = torch.broadcast_to(
                state.unsqueeze(-1), [*state.shape, frame.shape[-2]]
            )
            frame = torch.broadcast_to(
                frame.unsqueeze(1),
                [*state.shape, frame.shape[-1]],
            )
            return weight_fn(cache, frame, state)
        ...
        def gather_weight(weights, y):
            ...
            mask = F.one_hot(y.long() - 1, weights.shape[-1])
            return torch.einsum("...TV,...V->...T", weights, mask.float())

        def weight_step(weight_fn, inputs):
        ...
        context_next_labels = torch.cat(
            [labels, torch.ones_like(labels[..., :1])], dim=-1
        )

        weight_step = checkpoint(
            weight_step, self.weight_fn, (context_states, context_next_labels)
        )
        ...
        def scan(f, in_dims, out_dims):
            return lambda weight_fn, ctx_state, ctx_nxt_labels: weight_step(
                weight_fn, (context_states, context_next_labels)
            )

        # [batch_dims..., max_num_frames, max_num_labels+1]

        # _, (blank_weight, lexical_weight) = contexts.scan(weight_step)

        _, (blank_weight, lexical_weight) = contexts.scan(
            weight_step,
            in_dims=len(batch_dims),
            out_dims=len(batch_dims) + 1,
        )(self.weight_fn, context_states, context_next_labels)
        ...
        is_final = num_labels[..., None] == torch.arange(num_alpha_states)
        return semiring.sum(
    ...
    def _forward(
        ...
        if blank_mask is not None and len(blank_mask) != self.alignment.num_states():
            raise ValueError(
        ...
        if (
            lexical_mask is not None
            and len(lexical_mask) != self.alignment.num_states()
        ):
            raise ValueError(
        ...
        # backward, but still better than the defaults (i.e. no checkpoints or no saving
        # intermediates at all).
        #
        # # For the tropical semiring, this should be equivalent to no checkpoint.
        ...
        (
            _,
            alpha_T,
        ), alpha_0_to_T_minus_1 = contexts.scan(  # pylint: disable=invalid-name
            step,
            in_dims=len(batch_dims),
            out_dims=len(batch_dims),
        )(
            self.weight_fn, init_carry, inputs
        )
    ...
    class BackwardStepCallback(Protocol):
        ...
        def __call__(
            self,
            weight_vjp_fn: Callable[
                [Blank, Lexical], tuple[ParamsGrad, CacheGrad, FrameGrad]
            ],
            carry: Carry,
            blank_marginal: Blank,
            lexical_marginals: Lexical,
        ) -> tuple[Carry, Output]:
    ...
    def _backward(
        ...
        num_frames: torch.Tensor,
        log_z: torch.Tensor,
        alpha_0_to_T_minus_1: torch.Tensor,  # pylint: disable=invalid-name
        init_callback_carry: ...,
        ...
        if log_z.shape != batch_dims:
            raise ValueError(
                "log_z and num_frames have different batch_dims: "
        ...
        def step(lattice, carry, inputs):
            ...
            lexical_marginals = torch.where(is_padding[..., None], 0, lexical_marginals)
        ...
        init_beta = semirings.Log.ones([*batch_dims, num_context_states], log_z.dtype)
        init_t = torch.tensor(frames.shape[-2] - 1)
        init_carry = (init_t, init_beta, init_callback_carry)

        inputs = (alpha_0_to_T_minus_1, frames)
        (_, _, final_callback_carry), callback_outputs = contexts.scan(
            step,
            in_dims=len(batch_dims),
            out_dims=len(batch_dims),
            reverse=True,
        )(self, init_carry, inputs)

        return final_callback_carry, callback_outputs

    def _forward_backward(
        self, cache: T, frames: torch.Tensor, num_frames: torch.Tensor
    ) -> torch.Tensor:
        ...
        def f(lattice: "RecognitionLattice", cache: T, frames: torch.Tensor):
        ...
        def fwd(lattice: "RecognitionLattice", cache: T, frames: torch.Tensor):
            """Forward pass function evaluation."""
            (
                log_z,
                alpha_0_to_T_minus_1,
            ) = lattice._forward(  # pylint: disable=invalid-name,protected-access
                cache=cache, frames=frames, num_frames=num_frames, semiring=semiring
            )
            ...
            def apply_backward(params, **kwargs):
                ...
                return lattice.apply(
                    params, **kwargs, method=lattice._backward
        ...
        def bwd(res, g: torch.Tensor):
            (
                ...
                log_z,
                alpha_0_to_T_minus_1,
            ) = res  # pylint: disable=invalid-name
            ...
            def vjp_callback(weight_vjp_fn, carry, blank_marginal, lexical_marginals):
                ...
                params_grad_t, cache_grad_t, frame_grad_t = weight_vjp_fn(
                    (
                        ...
                        torch.unsqueeze(g, (-1, -2)) * lexical_marginals,
                    )
                )
                next_carry = map(
                    torch.add, (params_grad, cache_grad), (params_grad_t, cache_grad_t)
                )
                outputs = frame_grad_t
                return next_carry, outputs

            # Zero accumulators of params_grad/cache_grad.
            init_params_grad = map(torch.zeros_like, params)
            init_cache_grad = map(torch.zeros_like, cache)
...
def _init_context_state_weights(
    ...
    weights = torch.where(is_start, semiring.ones([], dtype), semiring.zeros([], dtype))
