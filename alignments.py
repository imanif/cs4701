...
def shift_down(
    x: torch.Tensor, semiring: semirings.Semiring[torch.Tensor]
) -> torch.Tensor:
    ...
    """
    return torch.cat([semiring.zeros((*x.shape[:-1], 1), x.dtype), x[..., :-1]], dim=-1)


def check_num_weights(
    alignment: TimeSyncAlignmentLattice,
    blank: Sequence[torch.Tensor],
    lexical: Sequence[torch.Tensor],
):
...
class FrameDependent(TimeSyncAlignmentLattice):
    ...
    def forward(
        ...
        return semiring.plus(
            ...
            context.forward_reduce(
                semiring.times(alpha[..., None], lexical[0]), semiring
            ),
        )
    ...
    def backward(
        ...
        log_scale = alpha - log_z[..., None]
        blank_marginal = torch.exp(blank_beta + log_scale)
        lexical_marginal = torch.exp(lexical_beta + log_scale[..., None])
        next_beta = semirings.Log.plus(
            blank_beta, semirings.Log.sum(lexical_beta, dim=-1)
        )
...
class FrameLabelDependent(TimeSyncAlignmentLattice):
    ...
    def __init__(self, max_expansions: int) -> None:
        super(FrameLabelDependent, self).__init__()
    ...
    def forward(
        ...
        for i in range(self.max_expansions):
            last = context.forward_reduce(
                semiring.times(last[..., None], lexical[i]), semiring
            )
    ...
    def backward(
        ...
        last = alpha
        for i in range(self.max_expansions):
            last = context.forward_reduce(last[..., None] + lexical[i], semirings.Log)
            lexical_alphas.append(last)

        # Corresponding backward paths.
        blank_marginals = []
        blank_log_scale = beta - log_z[..., None]
        ...
        for i in range(self.max_expansions):
            ...
            log_scale = lexical_alphas[j] - log_z[..., None]
            lexical_marginals.append(torch.exp(lexical_beta + log_scale[..., None]))
            next_beta = semirings.Log.plus(
                blank[j] + beta, semirings.Log.sum(lexical_beta, dim=-1)
            )
